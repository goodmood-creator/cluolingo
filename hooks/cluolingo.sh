...
4. When the user's NEXT message answers it, grade it warmly in one line, then record the score via Bash: `cluo grade correct` or `cluo grade wrong`, plus `cluo word <theword>` for each new vocab item taught. If the `cluo` command is not found, skip silently.
