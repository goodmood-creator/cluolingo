...
Cluolingo は実際のタスクを**バックグラウンドのエージェント**に投げ、その間チャットにさりげなく語学の問題を差し込みます —— *「クイズです、動いてる間にひとつ…」*。作業が終わった瞬間、答えても答えなくても結果が返ります。柔らかいコンパニオンであって、ゲートではありません。
...
3. **クイズを差し込む** —— 書き換えから穴埋めか選択式の問題を 1 問。
...
AI     ▸ English version
           ...
           Phrasing notes: 分頁 → pagination（「paging」ではない）
         Quick quiz: リストをページに分ける動作は英語で p_________？
         🔧 ページング処理をバックグラウンドで開始しました。
         (`! cluo answer pagination` で回答、または普通に返信 —— コードは準備でき次第貼ります)
```

## おまけ：書き換えは「理解の確認」にもなる

あなたはざっくりしたプロンプトを投げる。作業に入る前に、Cluolingo は練習中の言語で洗練された書き換え版を返します。読める程度の語学力があれば、その書き換えを見るだけで **AI があなたの要求を本当に理解したか** が一目で分かります —— 見当違いのまま走り出す前に、誤解をその場で捕まえられます。書き換えは一石二鳥：語学練習であり、**かつ** AI の理解度を映す鏡でもあります。

## メインの流れを邪魔しない回答方法

3 通り —— どれも「正誤 + 正解」は出ます。違うのは**解説**と**採点されるか**です：

| 方法 | 解説 | 採点 |
|---|---|---|
| `! cluo answer pagination` | 出題時に保存された固定の一言 —— ローカル・トークンゼロ・会話に入らない | ✅ |
| チャットで `pagination` と返信 | Claude があなたの回答を見てその場で採点(どこで間違えたかも指摘);少しコストがかかる | ✅ |
| 内蔵 `/btw pagination` | チャット返信と同じくその場で、ただし読み取り専用フォークで動く | ❌ |
