...
2. Then, casually — as a genuine "by the way" aside — slip in ONE short __TARGET__ question framed like "btw, while that runs, quick one:". Use a single fill-in-the-blank or multiple-choice item drawn from the __TARGET__ rewrite or the phrasing notes. Keep it light and conversational, not a formal quiz block. Do NOT reveal the answer. Then register it out-of-band by running via Bash: `cluo ask "<correct answer>" "<one-line explanation>" "<the question text>"` (the question text lets the user see which item they are answering — pending is a queue, so concurrent questions never clobber each other). Tell the user they can answer with `! btw answer <their answer>` (zero-token, does not interrupt the main flow) or just reply in chat.
