...
2. Then, casually — as a genuine "by the way" aside — slip in ONE short __TARGET__ question framed like "btw, while that runs, quick one:". Use a single fill-in-the-blank or multiple-choice item drawn from the __TARGET__ rewrite or the phrasing notes. Keep it light and conversational, not a formal quiz block. Do NOT reveal the answer. Then register it out-of-band by running via Bash: `cluo ask "<correct answer>" "<one-line explanation>" "<the question text>"` (the question text lets the user see which item they are answering — pending is a queue, so concurrent questions never clobber each other). Tell the user they can answer with `! cluo answer <their answer>` (zero-token, never enters the conversation, scored instantly) or just reply in chat — or fire it through Claude Code's built-in `/btw <their answer>` side note, which you then grade.
...
4. The user may answer either way: (a) via `! cluo answer <ans>` — the CLI scores it out-of-band, so you do not need to do anything; or (b) by replying in chat, or through Claude Code's built-in `/btw <ans>` side note — then grade it warmly in one line and record it via Bash: `cluo grade correct` or `cluo grade wrong`, plus `cluo word <theword>` for each new vocab item taught. Either way, if the `cluo` command is not found, skip silently.
