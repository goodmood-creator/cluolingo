...
- **workout-gate** はウェブカメラの前で腕立て伏せを終えるまでプロンプトを凍結します。硬いゲートです。
...
AI     ▸ English version
         ...
         (`! btw answer pagination` で回答、または普通に返信 —— コードは準備でき次第貼ります)
```
...
- **アウトオブバンド（推奨・ゼロ干渉）：** `! btw answer pagination`。`!` シェル接頭辞はトークンを消費せず会話にも入らないため、メインのタスクを汚しません。CLI が即座に採点し、連勝を更新します。
- **チャット内（解説が欲しいとき）：** そのまま `pagination` と返信。Claude が一言の解説つきで温かく採点します。少しだけコンテキストを使いますが、学びは深いです。

`btw` と `cluo` は**同じコマンド** —— 読みやすい方をどうぞ（`! btw answer …`、`! cluo stats`）。
...
`cluo` + `btw` CLI を PATH にリンクし、フックを `~/.claude/settings.json` に組み込みます。いつでも再実行可能。アンインストールは `./install.sh --uninstall`。

> [`jq`](https://jqlang.github.io/jq/) が必要です（`brew install jq`）。

## CLI コマンド（`cluo` / `btw`）

Claude Code 内でトークン消費ゼロの `!` 接頭辞で実行します（例：`! btw stats`）。
